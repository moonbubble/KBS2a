...
// hier onder in commentaar is de code die gebruikt kan worden in TspScherm als je deze per see wilt
...
public class BruteForce {
	...
	public BruteForce(Bestelling order) {
		BruteForce.initialRoute = order.getProducten();
	}
...
//				+ route.getOrder().get(0).meetAfstand(route.getOrder().get(orderGrootte - 1)) //hier wordt de afstand gemeten tussen het eerste item uit de route en het laatste
	...
	public List<Product> ontvangRoute(BruteForce bruteForce, ArrayList<Route> shortestRoutes) {
		int index = 0;
